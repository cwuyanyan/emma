    ...
    <div class="app-content need-scroll">
      <div class="pad10 bg-white listbox">
        ...
        <!-- <div class="item hei60 ubb bor-graydc clearfix">
                                    ...
                                    <div class="ftc-orange get-yzcode">获取验证码</div>
                                    <div class=" get-yzcode color9"><span class="ftc-orange">42秒</span>  后重新获取</div>
                                </div> -->

        <div class="item  ubb bor-graydc clearfix">
          <div class=" fz30 col3 fl weixin-name">微信昵称</div>  
          <div class="con con1 fl">
            <input class="lineheight" v-model="carholder" type="text" placeholder="请输入微信昵称"/>
          </div>
        </div>
        <div class="item clearfix">
            <div class=" fz30 col3 fl weixin-name">微信号</div>
          <div class="con con1 fl">
            <input class="lineheight" v-model="bankno" type="text" placeholder="请输入微信号"/>
          </div>
        </div>
      </div>

      <div class=" mar10 mt50 mb20">
        <mu-raised-button @click="capitalBindBank" label="立即绑定" labelClass='fz30 tx-c'
                          class="demo-raised-button submit-btn" fullWidth secondary/>
      </div>
    </div>
  ...
  export default {
    data() {
      ...
      return {
        bankname: "微信",
        bankno: "",
        banktype: 3,//整数类型，账户类型  1支付宝  2银行卡  3微信
        carholder: "",
        accountbranch: "",
        // mobile: "",
        isRun: false,
    ...
    methods: {
      ...
      capitalBindBank() {//提现账户绑定
        ...
        if (_this.core.validate.isNullOrEmpty(_this.carholder.trim())) {
          _this.core.platformCore.methods.toast("请输入微信昵称");
          return;
        }
        if (_this.core.validate.isNullOrEmpty(_this.bankno.trim())) {
          _this.core.platformCore.methods.toast("请输入微信号");
          return;
        }
        ...
        var postData = {
          ...
          mobile: _this.$route.params.phone
        };
